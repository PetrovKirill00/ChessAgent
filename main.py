...
def main():
    ...
    response_qs = [ctx.Queue(maxsize=RESPONSE_Q_MAXSIZE) for _ in range(NUM_SELFPLAY_WORKERS)]
            ...
            if pending and (len(pending) >= SERVER_MAX_INFERENCE_POSITIONS or (pending_deadline is not None and now >= pending_deadline)):
                ...
                with torch.no_grad():
                    ...
                    if IPC_SEND_ONLY_LEGAL and pending:
                        all_have_idxs = True
                        max_len = 0
                        for (_wid, _rid, _obs, _idxs) in pending:
                            if _idxs is None:
                                all_have_idxs = False
                                break
                            try:
                        ...
                        if all_have_idxs:
                            ...
                            if max_len <= 0:
                                ...
                                for i_req, (_wid, _rid, _obs, _idxs) in enumerate(pending):
                                    n = int(len(_idxs))
                                    legal_lens[i_req] = n
                                    if n:
                ...
                out_by_wid = {}
                for i_req, ((worker_id, rid, _obs, idxs), logits_full, v) in enumerate(zip(pending, pol_np, values)):
