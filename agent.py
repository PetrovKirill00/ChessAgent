...
class InferenceClient:
    ...
    def __init__(self, request_q, response_q, worker_id: int, stop_event=None):
    ...
    def submit(self, obs: np.ndarray, legal_idxs: Optional[List[int]] = None, *, _suppress_first_print: bool = False) -> int:
        ...
        if (not self._printed_first) and (not _suppress_first_print):
        ...
        if IPC_SEND_ONLY_LEGAL and legal_idxs is not None:
            ...
            payload = (self.worker_id, int(rid), obs_send, idxs_send)
        else:
            payload = (self.worker_id, int(rid), obs_send)
